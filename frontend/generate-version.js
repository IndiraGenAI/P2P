...
function generate(argv) {
    ...
    try {
        ...
        if (e.code === 'ENOENT') {
            console.error(`Error: incorrect path to version file. Path used: ${e.path}`);
        } else {
            console.log(e);
        }
        process.exit(1);
    ...
    try {
        ...
        if (e.code === 'ENOENT') {
            console.error(`Error: incorrect path to index.html. Path used: ${e.path}`);
        } else {
            console.log(e);
        }
        process.exit(1);
    ...
    loadedIndex('head').append(`<script>window.__APP_VERSION__ = "${appVersion}"; window.__APP_VERSION_FILE__ = "${argv.v}"</script>`);
