...
} from '@/state/invoiceSource/invoiceSource.action';
import {
  invoiceSourceMasterSelector,
  clearInvoiceSourceMessage,
...
interface IInvoiceSourceFilterValues {
  name?: string;
  status?: string;
}
...
export const InvoiceSourcePage = () => {
  const dispatch = useAppDispatch();
  const state = useAppSelector(invoiceSourceMasterSelector);
  ...
  const [formValues, setFormValues] = useState<IInvoiceSourceFilterValues>({});
  ...
  const [editingRecord, setEditingRecord] = useState<
    IInvoiceSourceRecord | undefined
  >(undefined);
  const [confirmDeleteRow, setConfirmDeleteRow] =
    useState<IInvoiceSourceDetails | null>(null);
  ...
  useEffect(() => {
    ...
    dispatch(searchInvoiceSourceData(dataConvertFromSearchParm()));
  }, [searchParams]);

  useEffect(() => {
    const data: IInvoiceSourceFilterValues = {};
    searchParams.forEach((value, key) => {
  ...
  useEffect(() => {
    if (state.createInvoiceSource.message) {
      if (state.createInvoiceSource.hasErrors)
        message.error(state.createInvoiceSource.message);
      else message.success(state.createInvoiceSource.message);
      dispatch(clearInvoiceSourceMessage());
    }
  ...
  useEffect(() => {
    if (
      ...
      state.invoiceSourcesData.hasErrors
    ) {
      message.error(state.invoiceSourcesData.message);
      dispatch(clearInvoiceSourceMessage());
    }
  }, [state.invoiceSourcesData.message, state.invoiceSourcesData.hasErrors]);

  const rows = state.invoiceSourcesData.data?.rows ?? [];
  const meta = state.invoiceSourcesData.data?.meta;
  const totalCount = meta?.itemCount ?? 0;
  const isLoading = state.invoiceSourcesData.loading;

  const isEdit = editingRecord !== undefined;
  const isSubmitting =
    state.createInvoiceSource.loading || state.editById.loading;
  ...
  const onFinish = (values: IInvoiceSourceFilterValues) => {
  ...
  const openEditDrawer = (row: IInvoiceSourceDetails) => {
    setEditingRecord({
      id: row.id,
      code: row.code ?? '',
      name: row.name ?? '',
  ...
  const handleFormSubmit = async (values: IInvoiceSourceRecord) => {
    if (isEdit && editingRecord) {
      ...
      if (editInvoiceSourceById.fulfilled.match(result)) {
      ...
      const result = await dispatch(
        createNewInvoiceSource(
          trimObject({
            code: values.code,
            name: values.name,
          }) as IInvoiceSourceDetails,
        ),
      );
      if (createNewInvoiceSource.fulfilled.match(result)) {
        setIsFormDrawerOpen(false);
        if (skip === 0) {
          refreshCurrent();
        } else {
  ...
  const handleToggleStatus = async (
    row: IInvoiceSourceDetails,
    checked: boolean,
  ) => {
    const result = await dispatch(
      updateInvoiceSourceStatus({ id: row.id, status: checked }),
    );
    if (updateInvoiceSourceStatus.fulfilled.match(result)) refreshCurrent();
  };

  const handleConfirmDelete = async () => {
    if (!confirmDeleteRow) return;
    const result = await dispatch(removeInvoiceSourceById(confirmDeleteRow.id));
    setConfirmDeleteRow(null);
    if (removeInvoiceSourceById.fulfilled.match(result)) {
  ...
  return (
    <div className="p-6 h-full">
        ...
        <div className="px-6 py-5 flex items-center justify-between flex-wrap gap-3 flex-shrink-0 border-b border-gray-100">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5">
              {totalCount} invoice source{totalCount === 1 ? '' : 's'} configured
            </p>
          </div>
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <Can
              ...
              a={Common.Modules.MASTER.INVOICE_SOURCE}
            >
              <button
                type="button"
              ...
              >
                <Plus size={14} /> New Invoice Source
              </button>
            </Can>
          </div>
        </div>
                ...
                <tr>
                  ...
                  >
                    <div className="flex flex-col items-center gap-2">
                      <FileSignature size={28} className="text-gray-300" />
                      <p>No invoice sources found.</p>
      ...
      <Drawer
        ...
        title="Filter Invoice Sources"
        subtitle="Narrow down the invoice source list"
        footer={
      ...
      <FormModal
        ...
        title={isEdit ? 'Edit Invoice Source' : 'Create New Invoice Source'}
        subtitle={
          isEdit
            ? 'Update the invoice source details and save your changes.'
...
export default InvoiceSourcePage;
