...
import type { IApplicantTypeAddProps } from './Add.model';

const rules = {
  code: [
    { required: true, message: 'Please enter applicant type code' },
    { max: 50, message: 'Code must be at most 50 characters' },
  ],
  name: [
    { required: true, message: 'Please enter applicant type name' },
    { min: 2, max: 100, message: 'Name must be 2-100 characters' },
  ],
};

const ApplicantTypeAdd = (props: IApplicantTypeAddProps) => {
  ...
  const onFinish = (values: { code: string; name: string }) => {
    ...
    const payload: IApplicantTypeRecord = {
      id: data?.id ?? 0,
      code: trimmed.code,
      name: trimmed.name,
      status: data?.status,
...
export default ApplicantTypeAdd;
