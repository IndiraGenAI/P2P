...
import type { ICostCenterAddProps } from './Add.model';

const rules = {
  code: [
    { required: true, message: 'Please enter cost center code' },
...
const CostCenterAdd = (props: ICostCenterAddProps) => {
  ...
  const onFinish = (values: { code: string; name: string }) => {
    ...
    const payload: ICostCenterRecord = {
      id: data?.id ?? 0,
      code: trimmed.code,
      name: trimmed.name,
      status: data?.status,
