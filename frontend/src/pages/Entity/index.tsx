...
import {
  ...
  searchEntityData,
  updateEntityStatus,
} from '@/state/entity/entity.action';
import {
  entityMasterSelector,
  clearEntityMessage,
} from '@/state/entity/entity.reducer';
import type { IEntityDetails } from '@/services/entity/entity.model';
...
type SortKey = 'code' | 'name' | 'business_unit' | 'created_date' | 'status';
type SortDir = 'ASC' | 'DESC';

interface IEntityFilterValues {
  name?: string;
  status?: string;
}
...
const rules = {
  name: [
    ...
    { min: 1, max: 150, message: 'Must be 1-150 characters' },
  ],
};
...
export const EntityPage = () => {
  ...
  const state = useAppSelector(entityMasterSelector);
  ...
  const [formValues, setFormValues] = useState<IEntityFilterValues>({});
  ...
  const [editingRecord, setEditingRecord] = useState<IEntityRecord | undefined>(
    undefined,
  );

  const [confirmDeleteRow, setConfirmDeleteRow] =
    useState<IEntityDetails | null>(null);
  ...
  useEffect(() => {
    ...
    dispatch(searchEntityData(dataConvertFromSearchParm()));
  }, [searchParams]);

  useEffect(() => {
    const data: IEntityFilterValues = {};
    searchParams.forEach((value, key) => {
  ...
  useEffect(() => {
    if (state.createEntity.message) {
      if (state.createEntity.hasErrors)
        message.error(state.createEntity.message);
      else message.success(state.createEntity.message);
      dispatch(clearEntityMessage());
    }
  ...
  useEffect(() => {
    if (state.entitiesData.message && state.entitiesData.hasErrors) {
      message.error(state.entitiesData.message);
      dispatch(clearEntityMessage());
    }
  }, [state.entitiesData.message, state.entitiesData.hasErrors]);

  const rows = state.entitiesData.data?.rows ?? [];
  const meta = state.entitiesData.data?.meta;
  const totalCount = meta?.itemCount ?? 0;
  const isLoading = state.entitiesData.loading;

  const isEdit = editingRecord !== undefined;
  const isSubmitting =
    state.createEntity.loading || state.editById.loading;
  ...
  const onFinish = (values: IEntityFilterValues) => {
  ...
  const openEditDrawer = (row: IEntityDetails) => {
    setEditingRecord({
      id: row.id,
      code: row.code ?? '',
      name: row.name ?? '',
      ...
      liability_distribution: row.liability_distribution ?? '',
      prepayment_distribution: row.prepayment_distribution ?? '',
      shipping_addresses: row.shipping_addresses ?? [],
      billing_addresses: row.billing_addresses ?? [],
      status: row.status,
    });
    setIsFormDrawerOpen(true);
  };

  const handleFormSubmit = async (values: IEntityRecord) => {
    if (isEdit && editingRecord) {
      const result = await dispatch(
        editEntityById({
          ...
          legal_entity: values.legal_entity ?? null,
          liability_distribution: values.liability_distribution ?? null,
          prepayment_distribution: values.prepayment_distribution ?? null,
          shipping_addresses: values.shipping_addresses ?? [],
          billing_addresses: values.billing_addresses ?? [],
        }),
      );
      if (editEntityById.fulfilled.match(result)) {
      ...
      const result = await dispatch(
        createNewEntity(
          trimObject({
            ...
            shipping_addresses: values.shipping_addresses ?? [],
            billing_addresses: values.billing_addresses ?? [],
          }) as unknown as IEntityDetails,
        ),
      );
      if (createNewEntity.fulfilled.match(result)) {
        setIsFormDrawerOpen(false);
        if (skip === 0) {
          refreshCurrent();
        } else {
  ...
  const handleToggleStatus = async (
    ...
    const result = await dispatch(
      updateEntityStatus({ id: row.id, status: checked }),
    );
    if (updateEntityStatus.fulfilled.match(result)) refreshCurrent();
  };

  const handleConfirmDelete = async () => {
    if (!confirmDeleteRow) return;
    const result = await dispatch(removeEntityById(confirmDeleteRow.id));
    setConfirmDeleteRow(null);
    if (removeEntityById.fulfilled.match(result)) {
  ...
  return (
    <div className="p-6 h-full">
        ...
        <div className="px-6 py-5 flex items-center justify-between flex-wrap gap-3 flex-shrink-0 border-b border-gray-100">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5">
              {totalCount} entit{totalCount === 1 ? 'y' : 'ies'} configured
            </p>
          </div>
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <Can I={Common.Actions.CAN_ADD} a={Common.Modules.MASTER.ENTITY}>
              <button
                type="button"
                  ...
                  <td className="px-4 py-4 border-b border-slate-100/80">
                    <div className="flex items-center gap-2">
                      <Can
                        ...
                        a={Common.Modules.MASTER.ENTITY}
                      >
                        <button
                          type="button"
                  ...
                  >
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <p className="text-xs text-gray-400">
                        Try clearing the filters or create a new entity.
                      </p>
                    </div>
                  </td>
                </tr>
      ...
      <Drawer
        ...
        subtitle="Narrow down the entity list"
        footer={
      ...
      >
        ...
        >
          ...
          >
            <Input
              placeholder="Enter entity name or code"
      ...
      <FormModal
        ...
        title={isEdit ? 'Edit Entity' : 'Create New Entity'}
        subtitle={
          isEdit
            ? 'Update the entity details and save your changes.'
