...
export const GstPage = () => {
  ...
  const state = useAppSelector(gstMasterSelector);
  ...
  const [formValues, setFormValues] = useState<IGstFilterValues>({});
  ...
  const [editingRecord, setEditingRecord] = useState<IGstRecord | undefined>(
    undefined,
  );
  const [confirmDeleteRow, setConfirmDeleteRow] = useState<IGstDetails | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    dispatch(searchGstData(dataConvertFromSearchParm()));
  }, [searchParams]);

  useEffect(() => {
    const data: IGstFilterValues = {};
    searchParams.forEach((value, key) => {
  ...
  useEffect(() => {
    if (state.createGst.message) {
      if (state.createGst.hasErrors) message.error(state.createGst.message);
      else message.success(state.createGst.message);
      dispatch(clearGstMessage());
    }
  ...
  useEffect(() => {
    if (state.gstsData.message && state.gstsData.hasErrors) {
      message.error(state.gstsData.message);
      dispatch(clearGstMessage());
    }
  }, [state.gstsData.message, state.gstsData.hasErrors]);

  const rows = state.gstsData.data?.rows ?? [];
  const meta = state.gstsData.data?.meta;
  const totalCount = meta?.itemCount ?? 0;
  const isLoading = state.gstsData.loading;

  const isEdit = editingRecord !== undefined;
  const isSubmitting = state.createGst.loading || state.editById.loading;
  ...
  const onFinish = (values: IGstFilterValues) => {
  ...
  const openEditDrawer = (row: IGstDetails) => {
    setEditingRecord({
      id: row.id,
      code: row.code ?? '',
      name: row.name ?? '',
  ...
  const handleFormSubmit = async (values: IGstRecord) => {
    if (isEdit && editingRecord) {
      ...
      if (editGstById.fulfilled.match(result)) {
      ...
      if (createNewGst.fulfilled.match(result)) {
        setIsFormDrawerOpen(false);
        const sp = new URLSearchParams(searchParams.toString());
        sp.set('skip', '0');
        setSearchParams(sp);
  ...
  const handleToggleStatus = async (row: IGstDetails, checked: boolean) => {
    const result = await dispatch(
      updateGstStatus({ id: row.id, status: checked }),
    );
    if (updateGstStatus.fulfilled.match(result)) refreshCurrent();
  };

  const handleConfirmDelete = async () => {
    if (!confirmDeleteRow) return;
    const result = await dispatch(removeGstById(confirmDeleteRow.id));
    setConfirmDeleteRow(null);
    if (removeGstById.fulfilled.match(result)) {
  ...
  return (
    <div className="p-6 h-full">
        ...
        <div className="px-6 py-5 flex items-center justify-between flex-wrap gap-3 flex-shrink-0 border-b border-gray-100">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5">
              {totalCount} GST rate{totalCount === 1 ? '' : 's'} configured
            </p>
          </div>
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <Can I={Common.Actions.CAN_ADD} a={Common.Modules.MASTER.GST}>
              <button
                type="button"
                ...
                <tr>
                  ...
                  >
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <p>No GST rates found.</p>
                      <p className="text-xs text-gray-400">
                        Try clearing the filters or create a new GST rate.
                      </p>
                    </div>
                  </td>
                </tr>
      ...
      <Drawer
        ...
        title="Filter GST"
        subtitle="Narrow down the GST list"
        footer={
      ...
      <FormModal
        ...
        title={isEdit ? 'Edit GST' : 'Create New GST'}
        subtitle={
          isEdit
            ? 'Update the GST rate details and save your changes.'
            : 'Add a new GST rate.'
        }
        footer={
