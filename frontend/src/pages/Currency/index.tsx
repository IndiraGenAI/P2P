...
} from '@/state/currency/currency.action';
import {
  currencyMasterSelector,
  clearCurrencyMessage,
...
type SortKey = 'code' | 'name' | 'symbol' | 'created_date' | 'status';
type SortDir = 'ASC' | 'DESC';

interface ICurrencyFilterValues {
  name?: string;
  status?: string;
}
...
export const CurrencyPage = () => {
  ...
  const state = useAppSelector(currencyMasterSelector);
  ...
  const [formValues, setFormValues] = useState<ICurrencyFilterValues>({});
  ...
  const [editingRecord, setEditingRecord] = useState<ICurrencyRecord | undefined>(
    undefined,
  );
  const [confirmDeleteRow, setConfirmDeleteRow] =
    useState<ICurrencyDetails | null>(null);
  ...
  useEffect(() => {
    ...
    dispatch(searchCurrencyData(dataConvertFromSearchParm()));
  }, [searchParams]);

  useEffect(() => {
    const data: ICurrencyFilterValues = {};
    searchParams.forEach((value, key) => {
  ...
  useEffect(() => {
    if (state.createCurrency.message) {
      if (state.createCurrency.hasErrors)
        message.error(state.createCurrency.message);
      else message.success(state.createCurrency.message);
      dispatch(clearCurrencyMessage());
    }
  ...
  useEffect(() => {
    if (state.currenciesData.message && state.currenciesData.hasErrors) {
      message.error(state.currenciesData.message);
      dispatch(clearCurrencyMessage());
    }
  }, [state.currenciesData.message, state.currenciesData.hasErrors]);

  const rows = state.currenciesData.data?.rows ?? [];
  const meta = state.currenciesData.data?.meta;
  const totalCount = meta?.itemCount ?? 0;
  const isLoading = state.currenciesData.loading;

  const isEdit = editingRecord !== undefined;
  const isSubmitting = state.createCurrency.loading || state.editById.loading;
  ...
  const onFinish = (values: ICurrencyFilterValues) => {
  ...
  const openEditDrawer = (row: ICurrencyDetails) => {
    setEditingRecord({
      id: row.id,
      code: row.code ?? '',
      name: row.name ?? '',
  ...
  const handleFormSubmit = async (values: ICurrencyRecord) => {
    if (isEdit && editingRecord) {
      ...
      if (editCurrencyById.fulfilled.match(result)) {
      ...
      if (createNewCurrency.fulfilled.match(result)) {
        setIsFormDrawerOpen(false);
        const sp = new URLSearchParams(searchParams.toString());
        sp.set('skip', '0');
        setSearchParams(sp);
  ...
  const handleToggleStatus = async (
    row: ICurrencyDetails,
    checked: boolean,
  ) => {
    const result = await dispatch(
      updateCurrencyStatus({ id: row.id, status: checked }),
    );
    if (updateCurrencyStatus.fulfilled.match(result)) refreshCurrent();
  };

  const handleConfirmDelete = async () => {
    if (!confirmDeleteRow) return;
    const result = await dispatch(removeCurrencyById(confirmDeleteRow.id));
    setConfirmDeleteRow(null);
    if (removeCurrencyById.fulfilled.match(result)) {
  ...
  return (
    <div className="p-6 h-full">
        ...
        <div className="px-6 py-5 flex items-center justify-between flex-wrap gap-3 flex-shrink-0 border-b border-gray-100">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5">
              {totalCount} currenc{totalCount === 1 ? 'y' : 'ies'} configured
            </p>
          </div>
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <Can I={Common.Actions.CAN_ADD} a={Common.Modules.MASTER.CURRENCY}>
              <button
                type="button"
                  ...
                  <td className="px-4 py-4 border-b border-slate-100/80">
                    <div className="flex items-center gap-2">
                      <Can
                        ...
                        a={Common.Modules.MASTER.CURRENCY}
                      >
                        <button
                          type="button"
                  ...
                  >
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <p className="text-xs text-gray-400">
                        Try clearing the filters or create a new currency.
                      </p>
                    </div>
                  </td>
                </tr>
      ...
      <Drawer
        ...
        subtitle="Narrow down the currency list"
        footer={
      ...
      <FormModal
        ...
        title={isEdit ? 'Edit Currency' : 'Create New Currency'}
        subtitle={
          isEdit
            ? 'Update the currency details and save your changes.'
            : 'Add a new currency.'
        }
        footer={
