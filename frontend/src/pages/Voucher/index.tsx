...
import {
  ...
  searchVoucherData,
  updateVoucherStatus,
} from '@/state/voucher/voucher.action';
import {
  voucherMasterSelector,
  clearVoucherMessage,
...
interface IVoucherFilterValues {
  name?: string;
  status?: string;
}
...
export const VoucherPage = () => {
  ...
  const state = useAppSelector(voucherMasterSelector);
  ...
  const [formValues, setFormValues] = useState<IVoucherFilterValues>({});
  ...
  const [editingRecord, setEditingRecord] = useState<IVoucherRecord | undefined>(
    undefined,
  );
  const [confirmDeleteRow, setConfirmDeleteRow] = useState<IVoucherDetails | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    dispatch(searchVoucherData(dataConvertFromSearchParm()));
  }, [searchParams]);

  useEffect(() => {
    const data: IVoucherFilterValues = {};
    searchParams.forEach((value, key) => {
  ...
  useEffect(() => {
    if (state.createVoucher.message) {
      if (state.createVoucher.hasErrors)
        message.error(state.createVoucher.message);
      else message.success(state.createVoucher.message);
      dispatch(clearVoucherMessage());
    }
  ...
  useEffect(() => {
    if (state.vouchersData.message && state.vouchersData.hasErrors) {
      message.error(state.vouchersData.message);
      dispatch(clearVoucherMessage());
    }
  }, [state.vouchersData.message, state.vouchersData.hasErrors]);

  const rows = state.vouchersData.data?.rows ?? [];
  const meta = state.vouchersData.data?.meta;
  const totalCount = meta?.itemCount ?? 0;
  const isLoading = state.vouchersData.loading;

  const isEdit = editingRecord !== undefined;
  const isSubmitting = state.createVoucher.loading || state.editById.loading;
  ...
  const onFinish = (values: IVoucherFilterValues) => {
  ...
  const openEditDrawer = (row: IVoucherDetails) => {
    setEditingRecord({
      id: row.id,
      code: row.code ?? '',
      name: row.name ?? '',
  ...
  const handleFormSubmit = async (values: IVoucherRecord) => {
    if (isEdit && editingRecord) {
      ...
      if (editVoucherById.fulfilled.match(result)) {
      ...
      if (createNewVoucher.fulfilled.match(result)) {
        setIsFormDrawerOpen(false);
        if (skip === 0) {
          refreshCurrent();
        } else {
  ...
  const handleToggleStatus = async (row: IVoucherDetails, checked: boolean) => {
    const result = await dispatch(
      updateVoucherStatus({ id: row.id, status: checked }),
    );
    if (updateVoucherStatus.fulfilled.match(result)) refreshCurrent();
  };

  const handleConfirmDelete = async () => {
    if (!confirmDeleteRow) return;
    const result = await dispatch(removeVoucherById(confirmDeleteRow.id));
    setConfirmDeleteRow(null);
    if (removeVoucherById.fulfilled.match(result)) {
  ...
  return (
    <div className="p-6 h-full">
        ...
        <div className="px-6 py-5 flex items-center justify-between flex-wrap gap-3 flex-shrink-0 border-b border-gray-100">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5">
              {totalCount} voucher{totalCount === 1 ? '' : 's'} configured
            </p>
          </div>
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <Can I={Common.Actions.CAN_ADD} a={Common.Modules.MASTER.VOUCHER}>
              <button
                type="button"
                  ...
                  <td className="px-4 py-4 border-b border-slate-100/80">
                    <div className="flex items-center gap-2">
                      <Can
                        ...
                        a={Common.Modules.MASTER.VOUCHER}
                      >
                        <button
                          type="button"
                  ...
                  >
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <p className="text-xs text-gray-400">
                        Try clearing the filters or create a new voucher.
                      </p>
                    </div>
                  </td>
                </tr>
      ...
      <Drawer
        ...
        subtitle="Narrow down the voucher list"
        footer={
      ...
      <FormModal
        ...
        title={isEdit ? 'Edit Voucher' : 'Create New Voucher'}
        subtitle={
          isEdit
            ? 'Update the voucher details and save your changes.'
            : 'Add a new voucher.'
        }
        footer={
