...
import { Eye, Filter, ListChecks, Loader2, Plus, ReceiptText } from 'lucide-react';
...
import grnInvoiceService from '@/services/grnInvoice/grnInvoice.service';
import type { IGrnDetail } from '@/services/grn/grn.model';
import type {
  IGrnInvoiceApprovalProgress,
  IGrnInvoiceApprovalStepRow,
  IGrnInvoicePayload,
  IGrnInvoiceRow,
  IGrnInvoiceStatusCounts,
} from '@/services/grnInvoice/grnInvoice.model';
import {
...
function invoiceCreatePrefillFromApprovedGrn(
  ...
  const r = grnDetailToRcRecordShape(detail);
  return {
    ...r,
    id: 0,
    rc_number: '',
...
function RcWorkflowStepCell({
  ...
  rcId: number;
  progress: IGrnInvoiceApprovalProgress | null | undefined;
}) {
  ...
  const [fetchedSteps, setFetchedSteps] = useState<
    IGrnInvoiceApprovalStepRow[] | null
  >(null);
...
const GrnInvoicePage = () => {
  ...
  const [rows, setRows] = useState<IGrnInvoiceRow[]>([]);
  ...
  const [createSourceGrnId, setCreateSourceGrnId] = useState<number | null>(null);
  ...
  const [statusCounts, setStatusCounts] = useState<IGrnInvoiceStatusCounts>({
    ALL: 0,
    PENDING: 0,
    APPROVED: 0,
    REJECTED: 0,
  ...
  const grnIdQuery = searchParams.get('grnId');
  useEffect(() => {
    if (!grnIdQuery) return;
    const id = Number.parseInt(grnIdQuery, 10);
    ...
    void grnService
      ...
      .then((api) => {
        ...
        if (String(detail.status ?? '').toUpperCase() !== RcStatus.APPROVED) {
          setPageToast({
            variant: 'error',
            text:
              'Only an approved GRN can be used to create an invoice.',
          });
          return;
        }
        const label = detail.grn_number?.trim() || `GRN #${id}`;
        setCreatePrefill(invoiceCreatePrefillFromApprovedGrn(detail));
        setCreateSourceGrnId(id);
      ...
      .catch((err) => {
        if (!cancelled) {
          setPageToast({
            variant: 'error',
            text: readApiErrorMessage(err, 'Could not load invoice.'),
          });
          setSearchParams(
            (prev) => {
              const next = new URLSearchParams(prev);
  ...
  const loadViewRecord = (id: number, options?: { clearRecord?: boolean }) => {
    ...
    grnInvoiceService
      ...
      .then((res) => {
        if (!res?.data) {
          const text = 'Could not load invoice.';
          setViewLoadError(text);
          setPageToast({ variant: 'error', text });
          return;
        }
        try {
          setViewRecord(grnInvoiceDetailToRcRecordShape(res.data));
        } catch (e) {
          const msg =
      ...
      .catch((err: unknown) => {
        const text = readApiErrorMessage(err, 'Could not load invoice.');
        setViewLoadError(text);
        setPageToast({ variant: 'error', text });
      })
  ...
  const submitRcApprovalDecision = async (
    ...
    try {
      const res = await grnInvoiceService.approvalDecision(viewRecord.id, {
        decision,
  ...
  const handleFormSubmit = async (values: IRateContractRecord) => {
    if (createSourceGrnId == null) {
      setPageToast({
        variant: 'error',
        text: 'Create invoice from an approved GRN (use Create Invoice on the GRN or open with ?grnId=).',
      });
      return;
    }
    const payload: IGrnInvoicePayload = {
    ...
    setSubmitting(true);
    try {
      const res = await grnInvoiceService.create(payload);
      setPageToast({
        variant: 'success',
        text: res.message ?? 'Invoice submitted.',
      });
      closeFormModal();
      refreshStatusCounts();
      const sp = new URLSearchParams(searchParams.toString());
      ...
      setPageToast({
        variant: 'error',
        text: readApiErrorMessage(err, 'Could not create invoice.'),
      });
    } finally {
      setSubmitting(false);
    }
  ...
  const STATUS_TABS: {
    key: '' | RcListTabStatus;
    label: string;
    countKey: keyof IGrnInvoiceStatusCounts;
    text: string;
    badge: string;
    activeRing: string;
  }[] = [
  ...
  return (
    <div className="p-6 h-full">
                    ...
                    <td className="w-[1%] whitespace-nowrap px-2 py-4 text-center border-b border-slate-100/80 align-middle">
                      ...
                      >
                        <button
                          ...
                          aria-label={`View invoice ${row.grn_invoice_number ?? row.id}`}
                        >
                  ...
                  >
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <p className="text-xs text-gray-400">
                        Try clearing the filters or create an invoice from an approved GRN.
                      </p>
                    </div>
                  </td>
                </tr>
      ...
      <FormModal
        ...
        subtitle={
          createPrefillLabel
            ? `Prefilled from approved GRN ${createPrefillLabel}. Adjust amounts or lines before submitting.`
            : 'Create an invoice from an approved GRN (open this page with ?grnId=).'
        }
        footer={
                ...
                <div className={RC_SECTION_DIVIDER}>
                  ...
                  <div className="rounded-2xl border border-gray-200/90 bg-white px-4 py-3.5 text-sm text-gray-700 shadow-sm ring-1 ring-gray-100/80">
                    <p className="text-xs text-gray-600 leading-relaxed m-0">
                      No approval queue is stored for this invoice yet. Ensure an
                      approval workflow exists for{' '}
                      <span className="font-medium">GRN Invoice</span> for the
