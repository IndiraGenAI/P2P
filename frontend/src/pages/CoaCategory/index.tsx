...
} from '@/state/coaCategory/coaCategory.action';
import {
  coaCategoryMasterSelector,
  clearCoaCategoryMessage,
...
interface ICoaCategoryFilterValues {
  name?: string;
  status?: string;
}
...
export const CoaCategoryPage = () => {
  const dispatch = useAppDispatch();
  const state = useAppSelector(coaCategoryMasterSelector);
  ...
  const [formValues, setFormValues] = useState<ICoaCategoryFilterValues>({});
  ...
  const [editingRecord, setEditingRecord] = useState<
    ICoaCategoryRecord | undefined
  >(undefined);
  const [confirmDeleteRow, setConfirmDeleteRow] =
    useState<ICoaCategoryDetails | null>(null);
  ...
  useEffect(() => {
    ...
    dispatch(searchCoaCategoryData(dataConvertFromSearchParm()));
  }, [searchParams]);

  useEffect(() => {
    const data: ICoaCategoryFilterValues = {};
    searchParams.forEach((value, key) => {
  ...
  useEffect(() => {
    if (state.createCoaCategory.message) {
      if (state.createCoaCategory.hasErrors)
        message.error(state.createCoaCategory.message);
      else message.success(state.createCoaCategory.message);
      dispatch(clearCoaCategoryMessage());
    }
  ...
  useEffect(() => {
    if (
      ...
      state.coaCategoriesData.hasErrors
    ) {
      message.error(state.coaCategoriesData.message);
      dispatch(clearCoaCategoryMessage());
    }
  }, [state.coaCategoriesData.message, state.coaCategoriesData.hasErrors]);

  const rows = state.coaCategoriesData.data?.rows ?? [];
  const meta = state.coaCategoriesData.data?.meta;
  const totalCount = meta?.itemCount ?? 0;
  const isLoading = state.coaCategoriesData.loading;

  const isEdit = editingRecord !== undefined;
  const isSubmitting =
    state.createCoaCategory.loading || state.editById.loading;
  ...
  const onFinish = (values: ICoaCategoryFilterValues) => {
  ...
  const openEditDrawer = (row: ICoaCategoryDetails) => {
    setEditingRecord({
      id: row.id,
      name: row.name ?? '',
      status: row.status,
  ...
  const handleFormSubmit = async (values: ICoaCategoryRecord) => {
    if (isEdit && editingRecord) {
      ...
      if (editCoaCategoryById.fulfilled.match(result)) {
  ...
  const handleToggleStatus = async (
    row: ICoaCategoryDetails,
    checked: boolean,
  ) => {
    const result = await dispatch(
      updateCoaCategoryStatus({ id: row.id, status: checked }),
    );
    if (updateCoaCategoryStatus.fulfilled.match(result)) refreshCurrent();
  };

  const handleConfirmDelete = async () => {
    if (!confirmDeleteRow) return;
    const result = await dispatch(removeCoaCategoryById(confirmDeleteRow.id));
    setConfirmDeleteRow(null);
    if (removeCoaCategoryById.fulfilled.match(result)) {
  ...
  return (
    <div className="p-6 h-full">
        ...
        <div className="px-6 py-5 flex items-center justify-between flex-wrap gap-3 flex-shrink-0 border-b border-gray-100">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5">
              {totalCount} categor{totalCount === 1 ? 'y' : 'ies'} configured
            </p>
          </div>
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <Can
              ...
              a={Common.Modules.MASTER.COA_CATEGORY}
            >
              <button
                type="button"
                ...
                <tr>
                  ...
                  >
                    <div className="flex flex-col items-center gap-2">
                      <Tags size={28} className="text-gray-300" />
                      <p>No COA categories found.</p>
      ...
      <Drawer
        ...
        title="Filter COA Categories"
        subtitle="Narrow down the category list"
        footer={
      ...
      <FormModal
        ...
        title={isEdit ? 'Edit COA Category' : 'Create New COA Category'}
        subtitle={
          isEdit
            ? 'Update the category details and save your changes.'
            : 'Add a new COA category.'
        }
        footer={
...
export default CoaCategoryPage;
