...
} from '@/state/purchaseOrder/purchaseOrder.action';
import {
  clearCurrentPurchaseOrder,
  clearPurchaseOrderMessage,
  purchaseOrderSelector,
} from '@/state/purchaseOrder/purchaseOrder.reducer';
...
import type {
  IPurchaseOrderApprovalProgress,
...
import {
  purchaseOrderRowToRateContractRecord,
  rateContractRecordToPurchaseOrderPayload,
} from './poFormAdapter';
import {
  PrStatus,
...
/** Same section rhythm as `PurchaseOrder/Add`. */
...
/** List column: where the PO sits in the approval chain. */
...
function ApprovalWorkflowStepsApiBody({
  ...
  prStatus: string;
  steps: IPurchaseOrderApprovalStepRow[];
}) {
...
function WorkflowStepCell({
  ...
  prId: number;
  status: string;
  progress: IPurchaseOrderApprovalProgress | null | undefined;
}) {
  ...
  const [fetchedSteps, setFetchedSteps] = useState<
    IPurchaseOrderApprovalStepRow[] | null
  >(null);
...
const useFkOptions = () => {
  ...
  const [coaOptions, setCoaOptions] = useState<SelectOption[]>([]);
  ...
  useEffect(() => {
    ...
    coaService
      .searchCoaData(searchParamsToRecord(params))
      .then((res) => {
        const rows = res.data?.rows ?? [];
        setCoaOptions(
          rows.map((r) => ({
...
export const PurchaseOrderPage = () => {
  ...
  const state = useAppSelector(purchaseOrderSelector);
  ...
  const [statusCounts, setStatusCounts] =
    useState<IPurchaseOrderStatusCounts>({
      ALL: 0,
      PENDING: 0,
      APPROVED: 0,
      REJECTED: 0,
  ...
  const loadViewRecord = (id: number, options?: { clearRecord?: boolean }) => {
    ...
    purchaseOrderService
      ...
      .then((res) => {
        if (!res?.data) {
          setViewLoadError('Could not load purchase order.');
          message.error('Could not load purchase order.');
          return;
        }
        try {
          setViewRecord(purchaseOrderRowToRateContractRecord(res.data));
        } catch (e) {
          const msg =
            e instanceof Error ? e.message : 'Invalid purchase order data.';
  ...
  const openViewPurchaseOrder = (id: number) => {
  ...
  const poApprovedForGrn =
    String(viewRecord?.status ?? '').toUpperCase() === PrStatus.APPROVED;
  ...
  useEffect(() => {
    ...
    dispatch(searchPurchaseOrderData(dataFromSearch()));
  }, [searchParams]);
  ...
  const handleFormSubmit = async (values: IRateContractRecord) => {
    const payload = rateContractRecordToPurchaseOrderPayload(values);
    const result = await dispatch(createNewPurchaseOrder(payload));
    if (result.meta.requestStatus === 'fulfilled') {
      closeFormDrawer();
      const sp = new URLSearchParams(searchParams.toString());
      sp.set('status', PrStatus.PENDING);
  ...
  const STATUS_TABS: {
    key: '' | PrListTabStatus;
    label: string;
    countKey: keyof IPurchaseOrderStatusCounts;
    text: string;
    badge: string;
    activeRing: string;
  }[] = [
  ...
  return (
    <div className="p-6 h-full">
        ...
        <div className="px-6 py-5 flex items-center justify-between flex-wrap gap-3 flex-shrink-0 border-b border-gray-100">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <Can
              ...
              a={Common.Modules.PROCUREMENT.PURCHASE_ORDER}
            >
              <button
                type="button"
        ...
        <div className="flex-1 overflow-auto relative pr-6">
          <table className="w-full border-separate border-spacing-0">
            ...
            <tbody>
              ...
              {rows.map((row, index) => {
                const status = (row.status ?? PrStatus.DRAFT) as PurchaseOrderStatus;
                  ...
                  >
                    ...
                    <td className="w-[1%] whitespace-nowrap px-2 py-4 text-center border-b border-slate-100/80 align-middle">
                      ...
                      >
                        <button
                          type="button"
                          onClick={() => openViewPurchaseOrder(row.id)}
                          ...
                          aria-label={`View purchase order ${row.po_number ?? row.id}`}
                        >
      ...
      <FormModal
        ...
        subtitle="Create a new purchase order with items."
        footer={
      ...
      >
        <RateContractAdd
          ...
          termsConditions={fkOptions.termsConditions}
          currencies={fkOptions.currencies}
          gstRates={fkOptions.gstRates}
          coaOptions={fkOptions.coaOptions}
        />
      </FormModal>
      ...
      <FormModal
        ...
        footer={
          <div className="flex items-center justify-end gap-3 flex-wrap">
            ...
            >
              {!poApprovedForGrn ? (
                <Tooltip title="Create GRN is available only after the purchase order is approved.">
                ...
                <button
                  ...
                  onClick={() => {
                    if (!viewRecord?.id) return;
                    navigate(`/purchase-order/grn?poId=${viewRecord.id}`);
                    closeViewModal();
                  }}
