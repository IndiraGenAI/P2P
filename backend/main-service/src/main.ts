...
async function bootstrap() {
  ...
  if (APP_ENV.isDev()) {
    const config = new DocumentBuilder()
      .addBasicAuth()
      .addBearerAuth()
      .setTitle('ERP - Main Service')
      .setDescription('Main Service APIs')
      .setVersion('1.0')
      .build();
  ...
  console.log(
    `[P2P-ORG] Main Service listening on http://localhost:${APP_ENV.port}${APP_ENV.prefix} (mode=${APP_ENV.mode})`,
  );
}
bootstrap();
